...
monkey_boxplot \<- ggplot(long_data, aes(x = conditions, y = WPM, fill = conditions)) +

geom_boxplot(alpha = .7) +

labs(

title = "Distribution of WPM by Experimental Condition",
...
ggplot(participant_means, aes(x = music, y = Mean_WPM, 
                              color = participants, group = participants)) +
  geom_line() + 
  geom_point() +
  facet_wrap(~ gum) + # this creates separate plots for each gum condition
  labs(
    title = "Individual Participant Patterns Across Conditions",
    subtitle = "Left box is No Gum, Right box is Yes Gum",
    x = "Music",
    y = "Mean WPM",
    color = "Participants" )
